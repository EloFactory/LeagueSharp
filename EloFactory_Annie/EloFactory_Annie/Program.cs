...
namespace EloFactory_Annie
{
    internal class Program
    {
        ...
        {
            Player.SetSkin(Player.BaseSkinName, Config.Item("Annie.SkinChanger").GetValue<bool>() ? Config.Item("Annie.SkinChangerName").GetValue<StringList>().SelectedIndex : Player.BaseSkinId);
        ...
        #region PlayerDamage
        public static float getComboDamage(Obj_AI_Hero hero)
        {
            double damage = 0;
        ...
        public static float getMiniComboDamage(Obj_AI_Hero hero)
        {
            double damage = 0;
        ...
        public static void RLogic()
        {
            ...
            {
                foreach (PredictionOutput RPrediction in ObjectManager.Get<Obj_AI_Hero>()
        ...
        public static void FlashRLogic()
        {
            if (R.IsReady() && FlashSlot.IsReady() && Player.Mana >= RMANA && StunUp)
            {
                ...
                .Where(x => x.IsValidTarget(R.Range + 425)).Select(x => R.GetPrediction(x, true)).Where(pred => pred.Hitchance >= HitChance.High && pred.AoeTargetsHitCount >= Config.Item("Annie.FlashREnemiesHitCount").GetValue<Slider>().Value))
                {
                    ...
                    Utility.DelayAction.Add(10, () => R.Cast(RPrediction.CastPosition));
                }
            }
        }
        #endregion
