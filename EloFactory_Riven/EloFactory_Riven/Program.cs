...
namespace EloFactory_Riven
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("Flee").AddItem(new MenuItem("Riven.FleeMode", "Flee Mode E Priority").SetValue(new StringList(new[] { "E 1st", "E After 1st Q", "E After 2nd Q", "E After 3rd Q" }, 2)));
        ...
        public static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (W.IsReady())
            {
                ...
                if (Game.ClockTime < 1300f && Config.Item("Riven.AutoWEarly").GetValue<bool>())
                {
                ...
                if (Game.ClockTime >= 1300f && Config.Item("Riven.AutoWLate").GetValue<bool>())
                {
        ...
        public static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (unit.Team != ObjectManager.Player.Team && ShouldUseOn >= 0f && unit.IsValidTarget(Q.Range + E.Range))
            {
                if (Config.Item("Riven.3rdQInterrupt").GetValue<bool>() && Q.IsReady() && Player.Distance(unit) <= E.Range + Q.Range && Player.GetBuffCount("RivenTriCleaveBuff") == 2)
                {
                    if (Player.Distance(unit) > Q.Range && Config.Item("Riven.EInterrupt").GetValue<bool>())
                    {
                        ...
                        {
                            Player.IssueOrder(GameObjectOrder.MoveTo, unit.ServerPosition);
                        }
                        return;
                    }
                ...
                case "RivenTriCleave":
                    ...
                    if (targetCancel.IsValid && targetCancel.Distance(Player.ServerPosition) <= Q.Range)
                    {
                        if (Player.Distance(targetCancel) < Player.AttackRange / 2)
                        {
                            ...
                            Utility.DelayAction.Add(100 + 100 - Game.Ping / 2,
                                () => Player.IssueOrder(GameObjectOrder.MoveTo, targetCancel.ServerPosition));
                    }
            ...
            if (args.SData.Name.Contains("Attack"))
            {
                if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
                {
                    ...
                    {
                        Utility.DelayAction.Add(50 + (int)(Player.AttackDelay * 100) + Game.Ping / 2 + Config.Item("Riven.QDelay").GetValue<Slider>().Value, delegate
                        {
                            ItemsActivator();
                        });
                    }
                    ...
                    if (Config.Item("Riven.HarassOnInitiate").GetValue<bool>() && (Player.Distance(targetHarass) < Q.Range + Player.AttackRange && Player.HealthPercent > Config.Item("Riven.MiniHpForHarass").GetValue<Slider>().Value))
                    {
                        ...
                        if (CanUseAA && CanMove)
                        {
                            if (!(CastedQ || CastedW || CastedE || UsedAA))
                            {
                                ...
                                {
                                    CanCastQ = false;
                                    Player.IssueOrder(GameObjectOrder.AttackUnit, targetHarass);
                                }
                            }
                        }
        ...
        public static void JungleClear()
        {
            ...
            if (CanUseAA && CanMove)
            {
                if (!(CastedQ || CastedW || CastedE || UsedAA))
                {
                    ...
                    {
                        CanCastQ = false;
                        Player.IssueOrder(GameObjectOrder.AttackUnit, MinionN);
                    }
                }
            }
