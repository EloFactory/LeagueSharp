...
namespace EloFactory_Udyr
{
    internal class Program
    {
        ...
        public static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (Config.Item("Udyr.AutoWWhenEnemyCast").GetValue<bool>() && (unit.IsValid<Obj_AI_Hero>() && !unit.IsValid<Obj_AI_Turret>()) && unit.IsEnemy && args.Target.IsMe && W.IsReady() && Player.Distance(unit) < 1000)
            {
        ...
        static void Orbwalking_BeforeAttack(LeagueSharp.Common.Orbwalking.BeforeAttackEventArgs args)
        {
            ...
            if (Config.Item("Udyr.AutoMuramana").GetValue<bool>())
            {
                ...
                if (args.Target.IsValid<Obj_AI_Hero>() && args.Target.IsEnemy && Items.HasItem(Muramanaitem) && Items.CanUseItem(Muramanaitem) && Player.ManaPercent > Config.Item("Udyr.AutoMuramanaMiniMana").GetValue<Slider>().Value)
                {
        ...
        private static float GetPassiveBuffTimer(Obj_AI_Hero Player)
        {
